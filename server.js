...
// If running on Vercel (serverless), avoid filesystem operations and disk-based multer
const isVercel = !!process.env.VERCEL;
if(!isVercel){
  // serve client files only when running as standalone server
  app.use(express.static(path.join(__dirname)));
}

const DATA_DIR = path.join(__dirname, 'data');
const IMAGES_DIR = path.join(__dirname, 'images');
// In non-serverless environments create directories if needed
if(!isVercel){
  try{ if(!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR,{recursive:true}); }catch(e){ console.warn('Could not create DATA_DIR', e && e.code); }
  try{ if(!fs.existsSync(IMAGES_DIR)) fs.mkdirSync(IMAGES_DIR,{recursive:true}); }catch(e){ console.warn('Could not create IMAGES_DIR', e && e.code); }
}else{
  console.log('Running in Vercel serverless environment: skipping filesystem setup');
}
